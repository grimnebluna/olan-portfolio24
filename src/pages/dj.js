...
const djPage = ({ data }) => {
  ...
  const filteredAndSortedPastGigs = data.allWpGig.edges
    .map(({ node }) => node)
    .filter(({ konzertfelder }) => {
      const gigDate = parseISO(konzertfelder.datum);
      return gigDate < currentDate; // Filter out gigs that have passed
  ...
  // Generate the additional content string
  const additionalContent = filteredAndSortedPastGigs.map(node => (
    ` / ${node.konzertfelder.ort}, ${node.title}`
  )).join('');

  // Combine the original history HTML with the additional content
  const combinedHtml = data.wpPage.historygigs.history + additionalContent;
    ...
    <Layout>
      <div className={styles.mainNormal}>
        ...
        <div className="zweispaltigWrapper">
          ...
          {/* Append filteredAndSortedPastGigs as string to data.wpPage.historygigs.history */}
